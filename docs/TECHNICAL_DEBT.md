...
|---|---|---|---|
| ① | **Optimizador por bisección de layer-offset** — desplazamiento de toda una capa como variable continua; minimiza la distancia ponderada de conectores buscando la raíz de la derivada (convexa, ~48 iter, forward/backward, conv. <0.001). El aporte más original/limpio de LAF. | `AlmaGag/layout/laf/position_optimizer.py:416-520` | 🟡 **PORTADO** como utilidad agnóstica `AlmaGag/layout/offset_optimizer.py::optimize_group_offsets` (grupos que se mueven juntos en un eje; `tests/test_offset_optimizer.py`, 6). **Hallazgo empírico**: cablearlo en `hier/columns.py` es **inerte** — hier ya empaqueta los carriles rectos a separación mínima, así que mover un carril entero no mejora nada (medido: 0 cambio en los 20 canónicos). Fue diseñado para el modelo de *puntos libres* de LAF; el destino natural es **AUTO** (placement con holgura real), pendiente de decisión. |
