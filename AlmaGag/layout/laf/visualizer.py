...
class GrowthVisualizer:
    ...
    def capture_phase7_iterative(
        ...
        Args:
            iterative_summary: Dict con max_depth, total_iterations, iterations[]
            structure_info: Información estructural
        """
    ...
    def _generate_phase7_iterative_svg(self, output_path: str) -> None:
        ...
        for idx, it in enumerate(iterations):
            ...
            for node_id, (ax, ay) in positions.items():
                ...
                # Nivel de centralidad efectiva
                # Muestra el alpha real usado en el ordering (incluyendo penalización
                # por hojas same-layer)
                if structure_info and effective_alphas:
                    eff_alpha = effective_alphas.get(node_id, 0.0)
                    if eff_alpha > 0:
                        score_text = f'α:{eff_alpha:.2f}'
                    else:
                        score_text = 'α:0'
                    dwg.add(dwg.text(
                        score_text,
    ...
    def _generate_phase11_final_svg(self, output_path: str) -> None:
        ...
        """
        snapshot = self.snapshots['phase11']
        layout = snapshot['layout']
        structure_info = snapshot.get('structure_info')
